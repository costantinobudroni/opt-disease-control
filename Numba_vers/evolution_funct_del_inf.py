...
###z, the derivatives with respect to deliberate infection
###week is the current week
@jit('types.Tuple((float64[:], float64[:,:], float64[:,:]))(int32, float64[:], float64[:,:], float64[:,:], float64, float64,  int32, float64[:])',nopython=True, cache=True)
def one_step_evol_del_inf_with_grad(t, x, y, z, confi, inoc, week, params):
    ...
    ##first, the part that does not depend on inoc explicitly
    ...
    zp[R] = params[gamma]*(z[I_R] ) + params[delta_h]*z[H_H] \
        + params[xi_c]*z[C_C]
    ##now, the part with the partial derivative of G^i
    ##infection directly from susceptible to exposed, no extra compartment
    zp[S, week] += - inocp*x[S]
    zp[E, week] += inocp*x[S]
...
@jit('float64[:](int32, float64[:], float64, float64,  float64[:])',nopython=True, cache=True)
...
def future_nat(x_0, t_0, t_f, params):
    ...
    for k in range(N):
        t = t_0 + k        
        x += one_step_evol(t, x, 0.0, 0.0, params)
    return x
...
def evolve(x_0, t_0, t_f, params, confinement, infection):
    ...
    for k in range(N - 1):
        ...
        x[:, k + 1] = x[:, k] + one_step_evol(t, x[:, k], confinement(t), infection(t), params)
    return x

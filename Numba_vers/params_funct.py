...
def ranges_params():
    ...
    params[phi, :] = -3.8
    params[nu, :] = 1/4.6 #Warning! I got this number from the text: it doesn't appear in the table of the paper
    params[r, :] = [0.0, 0.6]
    return params
    
    

## This function returns the parameter means found in the literature
...
## Generates random deviations with respect to average value, depending on 
## the number between 0 and 1, noise_level
@jit('float64[:](float64[:], float64[:], float64)', nopython=True, cache=True)
def rand_params_singv(aver, uncert, noise_level):
    ## add noise_level*random(-1/+1)*uncertainty interval to the average value
